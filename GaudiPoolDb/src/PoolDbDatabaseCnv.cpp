    ...
    if ( create ) {
      ...
      if ( status.isSuccess() )  {
        log << MSG::INFO << "Connect to existing Database file:" 
            << fname << " as " << oname << " for UPDATE" << endreq;
      }
    }
    else if ( read ) {
      if ( status.isSuccess() )  {
        log << MSG::INFO << "Connect to existing Database file:" 
            << fname << " as " << oname << " for READ" << endreq;
      }
    }
    else if ( recrea )  {
      status = saveDescription(fname, cntName, "File containing statistics results.",
                               "", guid, clid, "RECREATE");
      if ( status.isSuccess() )  {
        log << MSG::INFO << "Recreate Database file:" << fname << " as " << oname << endreq;
      }
    }
    else  {
      log << MSG::ERROR << "Don't know what to do:" << fname << endreq;
