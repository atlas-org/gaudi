...
StatusCode PoolDbBaseCnv::initialize()   {
  ...
  status = conversionSvc()->queryInterface(IID_IPoolDbMgr, (void**)ptr1);
  if ( !status.isSuccess() )    {
    log << MSG::ERROR << "Cannot connect to \"IPoolDbMgr\" interface." << endmsg;
    return status;
  }
  IDataManagerSvc **ptr2 = &m_dataMgr;
  status = dataProvider()->queryInterface(IID_IDataManagerSvc, (void**)ptr2);
  if ( !status.isSuccess() )    {
...
{
  MsgStream log(messageService(),"PoolDbBaseCnv");
...
PoolDbBaseCnv::createObj(IOpaqueAddress* pAddress, DataObject*& refpObject)
{
  try   {
    ...
    if ( pA )  {
      ...
      MsgStream log(messageService(),"PoolDbBaseCnv");
      log << MSG::ERROR << "createObj> Cannot access the object:"+tag << endreq;
      return StatusCode::FAILURE;
    }
