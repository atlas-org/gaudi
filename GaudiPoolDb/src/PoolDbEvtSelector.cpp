...
PoolDbEvtSelector::PoolDbEvtSelector(const std::string& name, 
                                     ISvcLocator* svcloc ) 
  : Service( name, svcloc),
    m_dbMgr(0), m_rootCLID(CLID_NULL)
{
...
StatusCode PoolDbEvtSelector::queryInterface(const InterfaceID& riid, void** ppvIf) {
...
StatusCode PoolDbEvtSelector::initialize()    {
  ...
  if( !status.isSuccess() ) {
    log << MSG::ERROR 
        << "Unable to localize interface IID_IPoolDbMgr from service:" 
        << m_cnvSvcName << endreq;
    return status;
  }
  // Get DataSvc
