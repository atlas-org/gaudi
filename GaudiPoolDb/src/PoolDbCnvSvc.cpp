...
: ConversionSvc( nam, svc, POOL_StorageType), 
...
/// Query interface
StatusCode 
PoolDbCnvSvc::queryInterface(const InterfaceID& riid, void** ppvInterface)   {
  if ( IID_IPoolDbMgr == riid )  {
    *ppvInterface = (IPoolDbMgr*)this;
    addRef();
    return S_OK;
  }
...
StatusCode PoolDbCnvSvc::initialize()  {
  ...
  if( !status.isSuccess() ) {
    log << MSG::ERROR 
        << "Unable to localize interface from service:IODataManager" << endreq;
    return status;
  }
...
StatusCode PoolDbCnvSvc::setDataProvider(IDataProviderSvc* pDataSvc)  {
  ...
  if (pDataSvc)  {
    StatusCode status = 
      pDataSvc->queryInterface(IID_IDataManagerSvc, pp_cast<void>(&m_dataMgr));
    if ( !status.isSuccess() )    {
