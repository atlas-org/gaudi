...
StatusCode ReadAlg::execute() {
  ...
  if ( evt != 0 )    {
    ...
    if ( myTracks != 0 )    {
      ...
      for(MyTrackVector::iterator k2=myTracks->begin(); k2 !=myTracks->end(); ++k2) {
        ...
        if ( !vtx )  {
          log << MSG::ERROR << "(1) Invalid origin vertex !" << endreq;
          continue;
        }
        trk = const_cast<MyTrack*>(vtx->motherParticle());
        if ( !trk )  {
          log << MSG::ERROR << "(2) Invalid mother particle vertex !" << endreq;
          continue;
        }
        vtx = const_cast<MyVertex*>(trk->originVertex());
        if ( !vtx )  {
          log << MSG::ERROR << "(3) Invalid origin vertex !" << endreq;
          continue;
        }
      }
    }
  ...
  if ( evt_root )   {
    log << MSG::INFO << "Type of /Event is:" << typeid(*(evt_root.ptr())).name() << endreq;
    return StatusCode::SUCCESS;
  }
  else  {
