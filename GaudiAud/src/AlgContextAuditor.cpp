...
namespace 
{
  ...
  {
    if ( 0 == ni ) { return 0 ; }
    SmartIF<IAlgorithm> alg ( IAlgorithm::interfaceID() , ni ) ;
    return alg ;
  }
}
// ============================================================================
...
StatusCode AlgContextAuditor::initialize()
{
  ...
  { 
    ...
    log << MSG::ERROR << "Unable to locate 'AlgContextSvc'" << sc << endreq ;
    m_svc = 0 ; 
    return sc ;  // RETURN 
  }               
  if ( 0 == m_svc     ) 
  { 
    MsgStream log ( msgSvc() , name() ) ;
    log << MSG::ERROR << "Invalid pointer to IAlgContextSvc" << endreq ;
