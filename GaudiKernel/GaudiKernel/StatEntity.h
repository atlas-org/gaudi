 ...
 *  class ChronoStatSvc but it also could be used in standalone 
...
public:
   ...
   *  @param minFlag the minimun value for flag
...
public:
   ...
   *  overwise it returns the value of "flagMean"
   * 
   *  @code 
   * 
   *  StatEntity& stat = ... ;
  ...
  /** General decrement operator fro the flag 
   *  Could be used for easy manipulation with StatEntity object:
   *  
   *  @code 
   *
   ...
   */      
  StatEntity& operator-- (int) { return --(*this) ; }
  /** Assignement from the flag
   *  The action: reset and the general inrement
...
namespace Gaudi
{
  namespace Utils
  {
     ...
     *  minor change the format of individual columns to the overal 
     ...
     *     - uncertanty in binomial efficiency [perCent] (double)
     *
     *  The printout is performed according to format, 
     *  specifed by parameter "format2" 
     * 
     ...
     *
     *  If the parameter "flag" is set to <c>true</c> <b>AND</b>
     *  the counter name, converted to the lowercase constains one 
