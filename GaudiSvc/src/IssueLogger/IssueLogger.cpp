...
IssueLogger::finalize() {
  ...
  MsgStream log ( msgSvc(), name() );
  log << MSG::DEBUG << "IssueLogger::finalize" << endreq;
...
IssueLogger::setupLevels(Property& prop) {
  ...
  if (sap == 0) {
	...
	<< "to a StringProperty (which it should be!)" << endreq;
    return;
  }
...
IssueLogger::setupStreams(Property& prop) {
  ...
  if (sap == 0) {
    MsgStream log ( msgSvc(), name() );
    log << MSG::ERROR << "Could not convert " << prop.name()
	<< "to a StringArrayProperty (which it should be!)" << endreq;
    return;
  }
  ...
  for (itr = sap->value().begin(); itr != sap->value().end(); ++itr) {
    if (connect(*itr).isFailure()) {
      MsgStream log ( msgSvc(), name() );
      log << MSG::ERROR << "Could not setup stream " << *itr << endreq;
    }
  }
