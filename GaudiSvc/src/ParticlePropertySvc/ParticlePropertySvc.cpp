...
// ============================================================================
const InterfaceID& ParticlePropertySvc::type() const 
...
StatusCode ParticlePropertySvc::initialize() 
{
  ...
  if ( sc.isFailure() ) 
  {
    log << MSG::ERROR << " Could not parse the file " << endreq ;
    return sc ;
  }
  if ( !m_particles.empty() ) 
  {
    ...
    if ( sc.isFailure() ) 
    {
      log << MSG::ERROR << " Could not treat particles! " << endreq ;
      return sc ;
    }
  }
  ...
  {
    log << MSG::INFO 
        << "Properties have beed redefined for "
...
{ 
  if ( !m_other.empty() ) 
  {
    MsgStream log( msgSvc() , name() ) ;
    log << MSG::INFO
        << "Additional Properties have beed read from files: "
...
/// queryInterface.
// =============================================================================
StatusCode ParticlePropertySvc::queryInterface
...
StatusCode ParticlePropertySvc::parse() 
{
  ...
  if ( m_namemap.size() == 0 ) 
  {
    ...
    log << MSG::ERROR 
        ...
        << " - Check you are using ParticleData.txt" << endreq;
  ...
  const MSG::Level        l ) const
{
  ...
  if ( 0 == o || 0 == n  ) 
  {
    log << MSG::WARNING << " ParticleProperty* point to NULL" << endreq ;
    return true ;                                                    // RETURN 
  }
  //
  bool result = false ;
