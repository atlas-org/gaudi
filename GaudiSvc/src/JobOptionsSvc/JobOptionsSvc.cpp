...
// ============================================================================
/** @file 
 *  implemenation file for class JobOptionsSvc
 */
// ============================================================================
...
StatusCode JobOptionsSvc::queryInterface(const InterfaceID& riid, 
  void** ppvInterface)
{
  if ( IID_IJobOptionsSvc == riid )  {
    *ppvInterface = static_cast<IJobOptionsSvc*>( this ) ;
  }
  else if ( IID_IProperty == riid ) {
    *ppvInterface = static_cast<IProperty*>( this ) ;
  }
  else  {
                                       ...
                                       const std::string& path )
{
  ...
  {
    my_log << MSG::INFO 
           << "Job options successfully read in from " << file << endreq;  
    fillServiceCatalogue();  
  }
  else
  {
    my_log << MSG::FATAL << "Job options errors."<< endreq;    
  }
  // ----------------------------------------------------------------------------
  for ( std::vector<Gaudi::Parsers::Message>::const_iterator 
          cur=messages.begin(); cur!=messages.end();cur++)
  {
    my_log << convertSeverity(cur->severity()) << cur->message() << endreq;
  }
  // ----------------------------------------------------------------------------
  return sc;
}
...
void JobOptionsSvc::dump( const std::string& file ) const
{
  ...
  if ( !out ) 
  {
    log << MSG::ERROR << "Unable to open dump-file \""+file+"\"" << endreq ;
    return ;                                                   // RETURN 
  }
  else 
  { log << MSG::INFO << "Properties are dumped into \""+file+"\"" << endreq ; }
