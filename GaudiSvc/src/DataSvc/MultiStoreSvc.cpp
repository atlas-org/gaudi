...
/**
 * @class MultiStoreSvc MultiStoreSvc.h GaudiKernel/MultiStoreSvc.h
 *
 ...
 * @version 1.0
*/
class MultiStoreSvc : public Service,
		                  virtual public IDataProviderSvc, 
                      virtual public IDataManagerSvc,
                      virtual public IPartitionControl
{
protected:
...
#define _CALL(P,F,ARGS) \
  ...
  virtual STATUS create(CSTR& nam, CSTR& typ, IInterface*& pPartition)  {
    ...
    if ( !sc.isSuccess() )  {
      ListItem item(typ);
      IService* isvc = 0;
      STATUS sc = serviceLocator()->getService(item.name(), isvc);
      if ( !sc.isSuccess() )  {
        SmartIF<ISvcManager> mgr(serviceLocator());
        if ( mgr )  {
          sc = mgr->createService(item.type(), item.name(), isvc);
        }
      }
      if ( sc.isSuccess() )  {
  ...
  virtual STATUS queryInterface( const InterfaceID& riid, void** ppvUnknown )  {
    if ( IDataProviderSvc::interfaceID().versionMatch(riid) )
      *ppvUnknown = (IDataProviderSvc*)this;
    else if ( IDataManagerSvc::interfaceID().versionMatch(riid) )
      *ppvUnknown = (IDataManagerSvc*)this;
    else if ( IPartitionControl::interfaceID().versionMatch(riid) )
...
//protected:
  ...
  : Service(name,svc), m_rootCLID(110), m_rootName("/Event"), 
