...
: Service(nam, svcLoc), m_appMgrUI(0)
{
...
StatusCode AppMgrRunable::queryInterface(const InterfaceID& riid, void** ppvInterface)   {
...
StatusCode AppMgrRunable::initialize()   {
  ...
  if ( sc.isSuccess() )     {
    sc = serviceLocator()->queryInterface(IID_IAppMgrUI, pp_cast<void>(&m_appMgrUI));
