...
StatusCode AIDATupleSvc::myTest()
{
  ...
  log << MSG::INFO << "Wellcome to myTest!" << endreq;
                                         ...
                                         AIDA::ITuple*& tObj )
{
  ...
  log << MSG::ERROR << "Could not retrieve tuple " << fullPath << endreq;
                                  ...
                                  int mode )
{
  ...
  if (status.isSuccess() ) {
    ...
    if (typ == "") {
      ...
      status = serviceLocator()->queryInterface(IID_IProperty,(void **)&appPropMgr );
      if( !status.isSuccess() ) {
        // Report an error and return the FAILURE status code
        log << MSG::ERROR << "Could not get PropMgr" << endreq;
        return status;
      }
      ...
      if ( !status.isSuccess() ) {
        log << MSG::ERROR << "Could not get NTuple Persistency format"
            << " from ApplicationMgr properties" << endreq;
        return status;
      }
      tmp_typ = sp.value();
  ...
  log << MSG::ERROR << "Cannot add " << ident << " invalid filename!" << endreq;
  return StatusCode::FAILURE;
...
DataObject* AIDATupleSvc::createPath( const std::string& newPath )
{
  ...
  if( 0 != sep ) {
    ...
    MsgStream log( msgSvc(), name() );
    log << MSG::ERROR << "Unable to create the tuple path" << endreq;
    return 0;
  }
...
StatusCode AIDATupleSvc::initialize()
{
  ...
  if( status.isSuccess() ){
    ...
    if( !status.isSuccess() ) {
      log << MSG::ERROR << "Unable to set NTuple data store root." << endreq;
      delete rootObj;
      return status;
    }
...
StatusCode AIDATupleSvc::finalize()
{
  ...
  log << MSG::ERROR << "Error finalizing AIDATupleSvc." << endreq;
  return status;
}


StatusCode AIDATupleSvc::queryInterface( const InterfaceID& riid,
                                         void** ppvInterface )
{
  if( riid == IID_IAIDATupleSvc ) {
    *ppvInterface = static_cast<IAIDATupleSvc*>(this);
  }
  else {
