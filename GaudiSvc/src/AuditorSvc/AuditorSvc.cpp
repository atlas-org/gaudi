...
IAuditor* AuditorSvc::newAuditor_( MsgStream& log, const std::string& name ) {
  ...
  if ( aud ) {
    ...
    log << MSG::WARNING << "Unable to retrieve factory for Auditor " << name << endreq;
  }
  ...
  for ( VectorName::iterator it = m_audNameList.begin(); it != m_audNameList.end(); it++ ) {
    ...
    if ( !findAuditor_( *it ) ) { // if auditor does not yet exist
      ...
      if ( aud != 0 ) {
        ...
        log << MSG::ERROR << "Error constructing Auditor " << *it << endreq;
...
StatusCode AuditorSvc::queryInterface( const InterfaceID& riid, void** ppvInterface ) {
  if ( IID_IAuditorSvc == riid )    {
    *ppvInterface = (IAuditorSvc*)this;
  }
  else if ( IID_IAuditor == riid )    {
    *ppvInterface = (IAuditor*)this;
  }
  else  {
