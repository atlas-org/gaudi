...
DataObject* HistogramSvc::createPath(CSTR newPath)  {
  ...
  if(0 != sep) {
    ...
    MsgStream log(msgSvc(), name());
    log << MSG::ERROR << "Unable to create the histogram path" << endreq;
    return 0;
  }
...
StatusCode HistogramSvc::initialize()   {
  ...
  if (status.isSuccess()) {
    ...
    if (!status.isSuccess()) {
      log << MSG::ERROR << "Unable to set hstogram data store root." << endreq;
      delete rootObj;
      return status;
    }
    ...
    if ( status.isSuccess() )   {
      ...
      log << MSG::ERROR << "Could not find HistogramPersistencySvc." << endreq;
      return status;
    }
  ...
  if ( !m_defs1D.empty() ) 
  {
    log << MSG::INFO << " Predefined 1D-Histograms: " << endreq ;
    for ( Histo1DMap::const_iterator ih = m_defs1D.begin() ; 
          m_defs1D.end() != ih ; ++ih ) 
    {
      log << MSG::INFO 
          << " Path='"       << ih->first  << "'" 
          << " Description " << ih->second << endreq ;
    }
  }
  return status;
}
...
//------------------------------------------------------------------------------
StatusCode HistogramSvc::queryInterface
(const InterfaceID& riid,void** ppvInterface)  {
  if(riid == IID_IHistogramSvc)  {
    *ppvInterface = static_cast<IHistogramSvc*>(this);
    addRef();
    return StatusCode::SUCCESS;
  }
...
StatusCode HistogramSvc::finalize     ()
{
  if ( !m_mods1D.empty() ) 
  {
    MsgStream log ( msgSvc () , name () ) ;
    log << MSG::DEBUG 
        << " Substituted histograms #" << m_mods1D.size() << " : " << endreq ;
