...
EventCollectionSelector::EventCollectionSelector(const std::string& name, ISvcLocator* svcloc ) 
  : Service( name, svcloc), m_tupleSvc(0)
{
  m_pAddrCreator = 0;
...
StatusCode EventCollectionSelector::initialize()    {
  ...
  status = serviceLocator()->service("EventPersistencySvc", m_pAddrCreator, true );
  if( !status.isSuccess() )   {
    log << MSG::ERROR << "Unable to locate IAddressCreator interface of " << "EventPersistencySvc" << endreq;
    return status;
  }
  status = serviceLocator()->service(m_tupleSvcName, m_tupleSvc, true );
  if( !status.isSuccess() )   {
    log << MSG::ERROR << "Unable to locate INTupleSvc interface of " << m_tupleSvcName << endreq;
    return status;
  }
  return status;
}
...
EventCollectionSelector::connectDataSource(const std::string& db, const std::string& /* typ */ )  const {
  ...
  SmartIF<IDataSourceMgr> svc(IID_IDataSourceMgr, m_tupleSvc);
...
EventCollectionSelector::connectTuple(const std::string& nam, const std::string& itName, NTuple::Tuple*& tup, NTuple::Item<IOpaqueAddress*>*& item)  const {
  ...
  if ( status.isSuccess() )    {
    ...
    if ( status.isSuccess() )   {
      ...
      log << MSG::ERROR << "Item " << itName << " is not part of the collection:" << top << endreq;
    }
    delete item;
    item = 0;
  }
    ...
    err << MSG::ERROR << "Cannot connect to collection:" << top << endreq;
  }
  tup = 0;
  return status;
}
...
EventCollectionSelector::connectStatement(const std::string& typ, const std::string& crit, INTuple* tuple)  const {
  ...
  if ( seltyp.length() > 0 || crit.length() > 0 )   {
    ...
    SmartIF<ISelectStatement> stmt(IID_ISelectStatement, 
      ROOT::Reflex::PluginService::Create<IInterface*>(seltyp, (IInterface*)serviceLocator()));
    if ( stmt.isValid( ) )    {
...
EventCollectionSelector::createContext(Context*& refpCtxt) const
{
  ...
  if( !status.isSuccess() )  {
    MsgStream log(msgSvc(), name());
    log << MSG::ERROR << "Unable to connect Collection file \"" << m_database << "\"" << endreq;
    return status;
  }
...
StatusCode EventCollectionSelector::queryInterface(const InterfaceID& riid, void** ppvInterface)  {
