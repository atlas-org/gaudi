...
public:
  ...
  /// Query interface @see IInterface 
  virtual StatusCode queryInterface 
  ( const InterfaceID& riid , void** ppvI )
  {
    // valid placeholder? 
    if ( 0 == ppvI ) { return StatusCode::FAILURE ; }  // RETURN 
    if ( ICounterSvc::interfaceID() == riid ) 
    {
      *ppvI = static_cast<ICounterSvc*>(this);
  ...
  ( const std::string& group , 
    const std::string& name  ) ;
  /** Remove all counters of a givem group. If no such counter exists
  ...
  Counter*& refpCounter  )
{
  ...
  // insert new counter with priper name into proper group:
...
// ===========================================================================
// Print the counter valuee for thre whole group of counters 
// ===========================================================================
StatusCode CounterSvc::print 
( const std::string& grp , 
  Printout& printer      ) const  
...
void CounterSvc::print () const 
{
  ...
  {
    log << MSG::ALWAYS 
        << "Number of counters : "  << _num << endreq 
        << m_header << endreq ;
  }  
  for ( CountMap::const_iterator i = m_counts.begin(); i != m_counts.end(); ++i )  
  { 
    for ( NameMap::const_iterator j = i->second.begin() ; i->second.end() != j ; ++j ) 
    {
      ...
      log <<
        ...
        ( j->first , i->first , *c , m_useEffFormat , m_format1 , m_format2 ) 
          << endreq ;
    }
  } 
}
