				      ...
				      void** ppvInterface) 
{
  ...
  if ( ppvInterface ) {
    *ppvInterface = 0;
    ...
    if ( IStatusCodeSvc::interfaceID().versionMatch(riid) )    {
      //    if ( riid == IID_StatusCodeSvc ) {
      *ppvInterface = static_cast<IStatusCodeSvc*>(this);
...
StatusCodeSvc::reinitialize() {
  ...
  MsgStream log( msgSvc(), name() );
  log << MSG::INFO << "reinitialize" << endreq;
...
StatusCodeSvc::finalize() {
  ...
  if (m_dat.size() > 0) {
    ...
    log << MSG::INFO << "listing all unchecked return codes:" << endreq;
