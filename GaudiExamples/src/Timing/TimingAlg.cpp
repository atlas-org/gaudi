...
namespace GaudiExamples 
{
  ...
  {
    // friend factory needed for isnatntiation
  ...
  protected:
    ...
    virtual ~TimingAlg() {}     // virtual & protected desctrustor
    // ========================================================================
  private:
    // ========================================================================
    ...
    TimingAlg& operator= ( const TimingAlg& ) ; // no assignement
    // ========================================================================
  protected:
    // ========================================================================    
    /// do sometiing CPU-intensive
    double doSomething() ;
    // ========================================================================
  private:
    // ========================================================================
...
StatusCode GaudiExamples::TimingAlg::execute  ()  // the execution of the algorithm 
{
  ...
  { // make some cpu consuming evaluations: 
    ...
    debug () << "the result of the first  loop is " << doSomething() << endreq ;
  }
  //
  { // make some cpu consuming evaluations: 
    Chrono chrono ( name() + ": the second loop" ,  chronoSvc() ) ;
    debug () << "the result of the second loop is " << doSomething() << endreq ;
  }
  //
  { // make some cpu consuming evaluations:
    Chrono chrono ( &m_chrono1 ) ;
    debug () << "the result of the third  loop is " << doSomething() << endreq ;
  }
  //
  { // make some cpu consuming evaluations:
    Chrono chrono (  m_chrono2 ) ;
    debug () << "the result of the fourth loop is " << doSomething() << endreq ;
  }
  //
  { // make some cpu consuming evaluations:
    m_chrono3.start () ;
    debug () << "the result of the 5th    loop is " << doSomething() << endreq ;
    m_chrono3.stop  () ;
  }
  //
  { // make some cpu consuming evaluations:
