...
: RConverter(clid, svc), m_ntupleSvc(0)         {
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
  ...
  if ( status.isSuccess() )   {
    status = serviceLocator()->getService( "NTupleSvc", IID_INTupleSvc,
                                           *pp_cast<IInterface>(&m_ntupleSvc));
  }
  return status;
}
...
//-----------------------------------------------------------------------------
{
  ...
  if (rAddr == 0) {
    log << MSG::ERROR << "Could not dynamic cast to RootObjAddress" << endreq;
...
//        << dec << " <> " << pReg->identifier() << endreq;
  ...
  if ( status.isSuccess() )   {
    RootObjAddress *rAddr = dynamic_cast<RootObjAddress*>( pAddress );
    if (rAddr == 0) {
      log << MSG::ERROR << "Could not cast to RootObjAddress" << endreq;
      return StatusCode::FAILURE;
    }
    INTuple* nt = 0;
