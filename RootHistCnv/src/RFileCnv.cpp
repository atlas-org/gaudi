...
//------------------------------------------------------------------------------
{
  ...
  if ( mode[0] == 'O' ) {
    ...
    log << MSG::ERROR << "don't know how to do this yet. Aborting." << endreq;
    exit(1);
    ...
    log << MSG::DEBUG << "creating ROOT file " << fname << endreq;
    ...
    log << MSG::ERROR << "Uknown mode to access ROOT file" << endreq;
    return StatusCode::FAILURE;
  ...
  //    if (! rfile->IsOpen() ) {
  //      log << MSG::ERROR << "Could not open file " << spar[0] << " for writing" << endreq;
  //      return StatusCode::FAILURE;
  //    }  
  //    ipar[0] = (unsigned long)rfile;
...
//-----------------------------------------------------------------------------
{
  ...
  if ( pFile != 0 && pFile->isOpen() )    {
    ...
    unsigned long* ipar = (unsigned long*)pAddress->ipar();
    if (findTFile(ooname,rfile).isFailure()) {
      log << MSG::ERROR << "Problems closing TFile " << ooname << endreq;
      return StatusCode::FAILURE;
    }
    
    rfile->Write(0,TObject::kOverwrite);
    if ( log.level() <= MSG::INFO ) {
      log << MSG::INFO << "dumping contents of " << ooname << endreq;
      rfile->Print();
    }
    rfile->Close();
    delete rfile;
    ...
    log << MSG::ERROR << "TFile " << ooname << " is not open" << endreq;
  }
  return StatusCode::FAILURE;
}
