...
C_dict::C_dict()
{
  ...
  meta1->addField("time",  "TimePoint", "Event creation Time", OffsetOf(Event,m_time));   
  ...
  meta1->addMethod("setRun", "Set run number", argnames1, Event_setRun );
 	...
 	MetaClass* meta2 = new MetaClass("TimePoint", "Abolute time (64-bit) in ns");
  meta2->addField("time", "longlong", "Time",OffsetOf(TimePoint,m_time));
