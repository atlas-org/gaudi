...
StatusCode GslSvc::initialize() 
{
  ...
  if( sc.isFailure() )
    { log << MSG::ERROR 
          << " Error in initialization of base class 'Service'"<< endreq;
    return sc;
    }
  ...
  if( 0 != handler )
    { log << MSG::VERBOSE
          ...
          << endreq; }
  else { log << MSG::INFO << " Error Handler is NULL" << endreq ; }
  ...
  if( !m_handlersTypeNames.empty() )
    {
      ...
      IToolSvc* toolsvc = 0 ;
      StatusCode sc = 
        serviceLocator()->service( "ToolSvc" , toolsvc , true );
      if ( sc.isFailure() )
        { log << MSG::ERROR << " Could not locate Tool Service! " << endreq ;
        return sc ; }
      if ( 0 == toolsvc   ) 
        { log << MSG::ERROR << " Could not locate Tool Service! " << endreq ;
        return StatusCode::FAILURE ; }      
      for( Names::const_iterator it = m_handlersTypeNames.begin() ;
           m_handlersTypeNames.end() != it ; ++it )
        {
          ...
          if( sc.isFailure() )
            { log << MSG::ERROR 
                  << " Could not retrieve tool '" << *it << "'"<< endreq ;
            return sc ; }
          if( 0 == eh )
            { log << MSG::ERROR 
                  << " Could not retrieve tool '" << *it << "'"<< endreq ;
 ...
 *  @see IService 
 *  @see IInterface
 *  @param iid the unique interface 
 *  @param ppi the placeholder to put the interface 
 *  @return status code 
 */
// ============================================================================
StatusCode GslSvc::queryInterface 
( const InterfaceID& iid , 
  void**             ppi ) 
{
  if     ( 0 == ppi                          ) { return StatusCode::FAILURE ; }
  if     ( IGslSvc::    interfaceID() == iid )
    { *ppi = static_cast<IGslSvc*>    ( this ); }
